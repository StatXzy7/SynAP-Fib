...
PACK_ROOT_FILES = ("QUEUE_STATE.json", "SEED_2026_GATE.json")
...
def write_reviewer_pack(experiment_root: str | Path) -> dict[str, Any]:
    ...
    for arm in "ABCDE":
        for relative in (
            f"seed_2026/{arm}/RUN_COMPLETE.json",
            f"seed_2026/{arm}/checkpoints/best.pt",
            f"seed_2026/{arm}/checkpoints/last.pt",
            f"selection_trajectories/seed_2026_{arm}.png",
            f"paper_plot_packs/seed_2026/{arm}/best_test/plot_manifest.json",
        ):
    ...
    lines = [
        "# AE_COR_v2 paper / reviewer response pack",
        "",
        ...
        "- `../seed_2026/<arm>/{best,last}/{val,test}/`：完整逐图预测和 canonical metrics。",
    ]
