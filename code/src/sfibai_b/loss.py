...
# Auxiliary-loss schedules. None means constant weights for the whole run
# (arms A-F, bit-identical to the frozen protocol). Arm G ramps both
# auxiliary weights linearly from 0 at the end of the burn-in phase
# (epoch 20) to their full value at epoch 40, testing whether delaying
# auxiliary supervision past early representation formation removes the
# max-endpoint degradation observed for E and F.
AUX_SCHEDULES: dict[str, tuple[int, int] | None] = {
    "A": None,
    "B": None,
    "C": None,
    "D": None,
...
def aux_scale_for_epoch(arm: str, epoch: int) -> float:
    """Scale factor in [0, 1] applied to both auxiliary loss weights."""
    ramp = AUX_SCHEDULES.get(arm.upper())
    if ramp is None:
        return 1.0
    start, full = ramp
    if epoch <= start:
...
class SFibAIObjective(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.arm = arm.upper() if arm is not None else None
        self.aux_scale = 1.0
    ...
    def set_epoch(self, epoch: int) -> float:
        """Set the auxiliary-loss scale for this epoch; returns the scale."""
        if self.arm is None:
            self.aux_scale = 1.0
        else:
            self.aux_scale = aux_scale_for_epoch(self.arm, int(epoch))
    ...
    def loss_components(
        ...
        weighted_position = self.aux_scale * self.lambda_position * position
        weighted_box = self.aux_scale * self.lambda_box * box["box_raw"]
        return {
            **grading,
            "position": position,
            "weighted_position": weighted_position,
            "aux_scale": outputs["logits"].new_tensor(self.aux_scale),
            **box,
