...
# Arms whose grading pathway consumes the gated residual injections.
# Arm F trains both auxiliary branches (identical modules, initialization,
# and losses to E) but never injects their predictions into grading,
# isolating auxiliary-task regularization from prediction injection.
# Arm G is E with a warm-up schedule on the auxiliary losses instead.
ARM_RESIDUAL_INJECTION: dict[str, bool] = {
    "A": False,
    "B": False,
    "C": True,
    "D": True,
...
class SFibAIModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if bool(inject_residuals) != ARM_RESIDUAL_INJECTION[normalized_arm]:
            raise ValueError(
                f"Arm {normalized_arm} requires inject_residuals="
                f"{ARM_RESIDUAL_INJECTION[normalized_arm]}"
            )
        ...
        self.inject_residuals = bool(inject_residuals)
...
def build_model(*, arm: str, seed: int, pretrained: bool = True) -> SFibAIModel:
    ...
    return SFibAIModel(
        ...
        inject_residuals=ARM_RESIDUAL_INJECTION[normalized_arm],
    )
