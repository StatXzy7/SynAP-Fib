...
from sfibai_b.protocol import ARMS, EXPERIMENT_ROOT, ROUND_NAME, SEEDS
...
def run_formal_queue() -> dict[str, Any]:
    ...
    try:
        for arm, seed in tasks:
            ...
            state["updated_at"] = _now()
            _atomic_json(state, state_path)
        gate = audit_seed_gate(root, seed=SEEDS[0])
        _atomic_json(gate, root / f"SEED_{SEEDS[0]}_GATE.json")
        state[f"seed_{SEEDS[0]}_gate"] = "PASS"
        state["updated_at"] = _now()
        _atomic_json(state, state_path)
